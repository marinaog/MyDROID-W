...
def traj_eval_and_plot(traj_est, traj_ref, plot_parent_dir, plot_name,printer):
    ...
    max_map=ape_statistics["max"], title="APE mapped onto trajectory, RMSE: %.3f cm" % (ape_statistics["rmse"] * 100))
...
def full_traj_fill(traj_filler, mapper, stream, fast_mode=False):
...
def full_traj_eval(traj_est, stream, printer, logger, plot_parent_dir, plot_name):
